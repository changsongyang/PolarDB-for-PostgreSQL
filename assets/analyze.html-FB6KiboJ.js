import{_ as l,r as t,o as p,c,d as s,a as n,b as e,e as r}from"./app-BxYZyqA7.js";const d={},u=n("h1",{id:"code-analysis-of-analyze",tabindex:"-1"},[n("a",{class:"header-anchor",href:"#code-analysis-of-analyze"},[n("span",null,"Code Analysis of ANALYZE")])],-1),v=r(`<h2 id="background" tabindex="-1"><a class="header-anchor" href="#background"><span>Background</span></a></h2><p>PostgreSQL 在优化器中为一个查询树输出一个执行效率最高的物理计划树。其中，执行效率高低的衡量是通过代价估算实现的。比如通过估算查询返回元组的条数，和元组的宽度，就可以计算出 I/O 开销；也可以根据将要执行的物理操作估算出可能需要消耗的 CPU 代价。优化器通过系统表 <code>pg_statistic</code> 获得这些在代价估算过程需要使用到的关键统计信息，而 <code>pg_statistic</code> 系统表中的统计信息又是通过自动或手动的 <code>ANALYZE</code> 操作（或 <code>VACUUM</code>）计算得到的。<code>ANALYZE</code> 将会扫描表中的数据并按列进行分析，将得到的诸如每列的数据分布、最常见值、频率等统计信息写入系统表。</p><p>本文从源码的角度分析一下 <code>ANALYZE</code> 操作的实现机制。源码使用目前 PostgreSQL 最新的稳定版本 PostgreSQL 14。</p><h2 id="statistics" tabindex="-1"><a class="header-anchor" href="#statistics"><span>Statistics</span></a></h2><p>首先，我们应当搞明白分析操作的输出是什么。所以我们可以看一看 <code>pg_statistic</code> 中有哪些列，每个列的含义是什么。这个系统表中的每一行表示其它数据表中 <strong>每一列的统计信息</strong>。</p><div class="language-sql line-numbers-mode" data-ext="sql" data-title="sql"><pre class="language-sql"><code>postgres<span class="token operator">=</span><span class="token comment"># \\d+ pg_statistic</span>
